...
func (a *Agent) GetUsage(ctx context.Context) (*core.UsageReport, error) {
	if _, err := exec.LookPath("claude"); err != nil {
		return nil, fmt.Errorf("ttadk: 'claude' CLI not found in PATH")
	}
...
func (a *Agent) runClaudeUsageProbe(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("ttadk: create usage temp dir: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("ttadk: start claude usage probe: %w", err)
	}
	...
	for {
		...
		case <-probeCtx.Done():
			...
			return "", fmt.Errorf("ttadk: timed out waiting for Claude Code /usage panel: %w", probeCtx.Err())
		case err := <-readDone:
			if err != nil {
				return "", fmt.Errorf("ttadk: read Claude Code /usage output: %w", err)
			}
		case <-processDone:
			if usageScreen != "" {
				return usageScreen, nil
			...
			if waitErr != nil {
				return "", fmt.Errorf("ttadk: Claude Code exited before /usage rendered: %w", waitErr)
			}
			return "", fmt.Errorf("ttadk: Claude Code exited before /usage rendered")
		case <-ticker.C:
			...
			if action := nextClaudeUsageProbeAction(lastScreen, &state, time.Now()); action != "" {
				if _, err := io.WriteString(ptmx, action); err != nil {
					return "", fmt.Errorf("ttadk: write Claude Code /usage probe input: %w", err)
				}
			}
		}
	}
...
func parseClaudeUsageReport(text string, now time.Time) (*core.UsageReport, error) {
	...
	if text == "" {
		return nil, fmt.Errorf("ttadk: Claude Code /usage produced empty output")
	}
...
func parseClaudeUsageWindow(lines []string, header string, windowSeconds int, now time.Time) (core.UsageWindow, error) {
	...
	if start < 0 {
		return core.UsageWindow{}, fmt.Errorf("ttadk: missing %s block in Claude Code /usage output", header)
	}
	...
	if usedPercent == nil {
		return core.UsageWindow{}, fmt.Errorf("ttadk: missing usage percentage in %s block", header)
	}
...
func detectClaudeUsageOutputError(screen, stderr string) error {
	...
	case strings.Contains(joined, "unknown command") && strings.Contains(joined, "/usage"):
		return fmt.Errorf("ttadk: this Claude Code version does not support /usage; please upgrade Claude Code")
	case strings.Contains(joined, "auth login") || strings.Contains(joined, "not logged in") || strings.Contains(joined, "sign in"):
		return fmt.Errorf("ttadk: Claude Code is not logged in for /usage; run `claude auth login`")
	case strings.Contains(joined, "/usage") && (strings.Contains(joined, "not available") || strings.Contains(joined, "not supported") || strings.Contains(joined, "subscription")):
		return fmt.Errorf("ttadk: current Claude account does not support /usage")
	default:
		return nil
	}
}
