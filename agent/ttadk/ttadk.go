...
type Agent struct {
	workDir          string
	cliBin           string   // CLI binary name or path (default: "ttadk")
...
func New(opts map[string]any) (core.Agent, error) {
	...
	// TTADK is a multi-tool launcher (ttadk claude | cursor | gemini | codex | ...).
	// To drive the underlying claude CLI in headless stream-json mode we call
	//   ttadk claude -a "--output-format stream-json --input-format stream-json ..."
	// and let ttadk forward the flags to claude via its -a/--args passthrough.
	cliBin := "ttadk"
	cliExtraArgs := []string{"claude"}
	cliArgsFlag := "-a"
	if cliPath, _ := opts["cli_path"].(string); cliPath != "" {
		...
		// user took over cli_path — clear the default passthrough flag too
		cliArgsFlag = ""
	}
	if flag, ok := opts["cli_args_flag"].(string); ok && flag != "" {
		cliArgsFlag = flag
	}
	...
	if !spawnOpts.IsolationMode() {
		if _, err := exec.LookPath(cliBin); err != nil {
			return nil, fmt.Errorf("ttadk: %q CLI not found in PATH, please install it first", cliBin)
		}
	}
...
func (a *Agent) Name() string           { return "ttadk" }
func (a *Agent) CLIBinaryName() string  { return a.cliBin }
func (a *Agent) CLIDisplayName() string { return "TTADK" }

func (a *Agent) SetWorkDir(dir string) {
	a.mu.Lock()
	defer a.mu.Unlock()
	a.workDir = dir
	slog.Info("ttadk: work_dir changed", "work_dir", dir)
}
...
func (a *Agent) SetModel(model string) {
	...
	defer a.mu.Unlock()
	a.model = model
	slog.Info("ttadk: model changed", "model", model)
}
...
func (a *Agent) SetReasoningEffort(effort string) {
	...
	slog.Info("ttadk: reasoning effort changed", "effort", a.reasoningEffort)
}
...
func (a *Agent) fetchModelsFromAPI(ctx context.Context) []core.ModelOption {
	...
	if err != nil {
		slog.Debug("ttadk: failed to fetch models", "error", err)
		return nil
	}
...
func (a *Agent) StartSession(ctx context.Context, sessionID string) (core.AgentSession, error) {
	...
	slog.Debug("ttadk: StartSession provider state",
		"activeIdx", activeIdx,
...
func (a *Agent) ListSessions(ctx context.Context) ([]core.AgentSessionInfo, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("ttadk: resolve work_dir: %w", err)
	}
	...
	if err != nil {
		...
		return nil, fmt.Errorf("ttadk: read project dir: %w", err)
	}
...
func (a *Agent) DeleteSession(_ context.Context, sessionID string) error {
	homeDir, err := os.UserHomeDir()
	if err != nil {
		return fmt.Errorf("ttadk: cannot determine home dir: %w", err)
	}
	absWorkDir, err := filepath.Abs(a.workDir)
	if err != nil {
		return fmt.Errorf("ttadk: resolve work_dir: %w", err)
	}
	projectDir := findProjectDir(homeDir, absWorkDir)
	if projectDir == "" {
...
func (a *Agent) GetSessionHistory(_ context.Context, sessionID string, limit int) ([]core.HistoryEntry, error) {
	...
	projectDir := findProjectDir(homeDir, absWorkDir)
	if projectDir == "" {
		return nil, fmt.Errorf("ttadk: project dir not found")
	}

	path := filepath.Join(projectDir, sessionID+".jsonl")
	f, err := os.Open(path)
	if err != nil {
		return nil, fmt.Errorf("ttadk: open session file: %w", err)
	}
	defer f.Close()
...
func (a *Agent) SetMode(mode string) {
	...
	slog.Info("ttadk: permission mode changed", "mode", a.mode)
}
...
func (a *Agent) AddAllowedTools(tools ...string) error {
	...
	slog.Info("ttadk: updated allowed tools", "tools", tools, "total", len(a.allowedTools))
	return nil
}
...
func (a *Agent) SetActiveProvider(name string) bool {
	...
	if name == "" {
		a.activeIdx = -1
		slog.Info("ttadk: provider cleared")
		return true
	}
	for i, p := range a.providers {
		if p.Name == name {
			a.activeIdx = i
			slog.Info("ttadk: provider switched", "provider", name)
			return true
		}
	}
	return false
